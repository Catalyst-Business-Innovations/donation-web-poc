  ...
  printLabel(c: Container): void {
    const contentsHtml =
      c.contents.length > 0
        ? c.contents
            .map(
              x => `<tr><td>${x.categoryName}</td><td>${x.quantity}</td><td>${this.getCondLabel(x.condition)}</td></tr>`
            )
            .join('')
...
<div class="chips">
  ...
  <span class="chip" style="background:#dcfce7;color:#166534;border-color:#86efac">${statusLabel(c.status)}</span>
  ${c.destination ? `<span class="chip" style="background:#fef3c7;color:#92400e;border-color:#fcd34d">${destLabel(c.destination)}</span>` : ''}
</div>
<div class="grid">
